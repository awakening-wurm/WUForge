...
public abstract class ModLoaderShared<T extends Versioned> implements Versioned {
    ...
    private List<ModInfo> discoverMods(Path modDir) throws IOException {
        ...
        try(DirectoryStream<Path> directoryStream = Files.newDirectoryStream(modDir,"*.properties")) {
            for(Path modInfo : directoryStream) {
                String fileName = modInfo.getFileName().toString();
                String modName = fileName.substring(0,fileName.length()-11);
        ...
        try(DirectoryStream<Path> directoryStream = Files.newDirectoryStream(modDir,filterModsDir(handled))) {
            for(Path modInfo : directoryStream) {
    ...
    private DirectoryStream.Filter<? super Path> filterModsDir(final Set<String> handled) {
        return path -> Files.isDirectory(path) && !handled.contains(path.getFileName().toString());
    }
    ...
    public List<? extends ModEntry<T>> loadModsFromModDir(Path modDir) throws IOException {
        ...
        final ModInstanceBuilder<T> entryBuilder = new ModInstanceBuilder<T>(modClass);
        final DependencyResolver<ModInfo> dependencyResolver = new DependencyResolver<ModInfo>().provided(Collections.singleton(modLoaderProvided));
        final List<Entry> mods = dependencyResolver.order(unorderedMods).stream().map(modInfo -> {
        ...
        mods.stream().forEach(modEntry -> {
            ...
            logger.info(String.format("Loading %1$s as %2$s (%3$s)",modEntry.mod.getClass().getName(),
                                      modEntry.getName(),implementationVersion));
        });

        // new style mods with initable will do configure, preInit, init
        mods.stream().filter(modEntry -> filterInitableModEntry(modEntry)).forEach(modEntry -> {
        ...
        mods.stream().filter(modEntry -> filterNotInitableModEntry(modEntry)).forEach(modEntry -> {
            try(EarlyLoadingChecker c = EarlyLoadingChecker.init(modEntry.getName(),"configure")) {
                ((Configurable)modEntry.mod).configure(modEntry.getProperties());
            }
        });
    ...
    private boolean filterInitableModEntry(Entry modEntry) {
        return (modEntry.mod instanceof Initable || modEntry.mod instanceof PreInitable) && modEntry.mod instanceof Configurable;
    }

    private boolean filterNotInitableModEntry(Entry modEntry) {
        return !(modEntry.mod instanceof Initable || modEntry.mod instanceof PreInitable) && modEntry.mod instanceof Configurable;
    }
